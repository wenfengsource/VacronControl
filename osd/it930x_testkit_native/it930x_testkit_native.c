...
void Endeavour_Lock_Channel(int handle)
{
...
void Get_Packet(int handle, int target_times, int is_save){
	...
	while(get_times < target_times){
		...
		if(BufferIsEmpty)
		{
			return_length = DTV_GetData(handle, read_length, BlockBuffer);
			...
			ReadCounter = 0;	//Read from buffer begin;
		}					
		
		buffer = BlockBuffer + (ReadCounter * 188);
		
		ReadCounter++;
		
		if (ReadCounter == ReadTime)
		{
			BufferIsEmpty = True;
		}	
#else
		...
		if(is_save) {
			...
			fwrite(buffer, 1, READBUFFER_SIZE, save_file);
		}
	}//while
...
int main(int argc, char **argv)
{
	...
	while(menu_flag){
		...
		printf("\n3. Get TS Data & Analyze Packets  ");
		printf("\n4. Omega Set PID Filter      ");
		printf("\n5. Mutil-Channel Lock Test   ");
		printf("\n6. Read/Write Register       ");	
		printf("\n7. NULL Packet FIlter");
		printf("\n8. EEPROM Editor");
		printf("\n9. Endeavour_Get_Board_Input_Power");
		printf("\n10. RX Reset!");
		printf("\n11. Get Board ID");
		printf("\n99. Print IT9133 Info Register        ");
		printf("\n0. Quit                      ");
		printf("\n==============================================\n");
		printf("=> Please Input Your Choice: ");		
		...
		switch(chose_function){
			case 1:
				Endeavour_Lock_Channel(handle);
				break;
		
			case 2:
				Endeavour_Get_Statistic_Menu(handle);
				break;
		
			case 3:
				Endeavour_Get_Packet_Menu(handle);
				break;
			
			case 4:
				Endeavour_Set_PID_Filter(handle);
				break;
			
			case 5:
				Endeavour_Lock_Multi_Channel(handle);
				break;
				
			case 6:
				Endeavour_RW_Register(handle);
				break;
				
			case 7:
				printf("\n=> NULL Packet Filter(0: Disable, 1: Enable): ");
				res = scanf("%d", &ctrlflag);
				
				if(ctrlflag != 1 && ctrlflag != 0) {
					printf("\n=> Wrong input !");
					break;
				}
				...
				if(ctrlflag == 1)
					...
					Endeavour_NULLPacket_Filter(handle, 0);
				break;
			
			case 8:
				Endeavour_EEPROM_editor_menu(handle);
				break;
				
			case 9:
				Endeavour_Get_Board_Input_Power(handle);
				break;
			...
			case 10:
				...
				break;
			case 11:
				Endeavour_Get_Board_ID(handle);
				break;		
			case 99:
				Endevour_Print_registers(handle);
				break;
	...
	Endeavour_Get_Packet_Menu(handle);		
#endif
printf("[%s] test and release \n", __func__);	
system("dmesg");
	DTV_DeviceClose(handle);
    while(1){sleep(1);};	
EXIT:
	return 0;
}
