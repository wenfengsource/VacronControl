...
{ 
	...
	Byte	var[2];
	Byte	btmp = 0;
	int i = 0, j = 0, tmpnumber = 0, br_idx = 0, ts_idx = 0;
	...
	else
	{
		...
		for (br_idx = 0; br_idx < DC->it9300_Number; br_idx++)
		{
			...
			for(ts_idx = 0; ts_idx <  DC->it9300.receiver_chip_number[br_idx] + tmpnumber; ts_idx++)	/* +1(tmpnumber) for TsPort_0 */
			{
				//Receiver chip type
				if (br_idx == 0)
				{
...
#if 0
				//for TsPort 0
				DC->it9300.tsSource[0][4].tsPktLen = 188;	///Unknow
				DC->it9300.tsSource[0][4].initialized = False;	///Unknow
				DC->it9300.tsSource[0][4].existed =True;
				DC->it9300.tsSource[0][4].htsDev = &DC->it9133[0][4];				
				DC->it9133[0][4].userData = &DC->it9300;				
				DC->it9133[0][4].pendeavour = &DC->it9300;				
				DC->it9133[0][4].br_idx = 0;				
				DC->it9133[0][4].tsSrcIdx = 4;			
#endif			
...
Dword DRV_IT930x_Initialize (Device_Context *DC, Byte br_idx) {
    ...
    if(br_idx == 0)
    {
		...
		if(DC->it9300.gator[0].booted){
			/* getFirmwareVersion() */
			error = DRV_getFirmwareVersionFromFile(DC, &fileVersion);
			if (error)
			{
			...
			//deb_data("\t cmdVersion = 0x%x\n", cmdVersion);		
			//deb_data("\t fileVersion = 0x%x\n", fileVersion);		
			
			if (cmdVersion != fileVersion)
			{
				deb_data("\t Outside_Fw = 0x%lX, Inside_Fw = 0x%lX, Reboot\n", fileVersion, cmdVersion);
				
				error = IT9300_reboot(&DC->it9300, 0);
				DC->bBootCode = true;     
				if (error)
				{
			...
			{
				deb_data("\t Firwarm version is the same\n");
			}
    ...
    if(br_idx == 1)
    {
		error = IT9300_getFirmwareVersion (&DC->it9300, 1);
		if (error) 
		{	
			deb_data("IT9300_getFirmwareVersion fail! \n");
			goto exit;
		...
		if (DC->it9300.firmwareVersion != 0)
		{
			DC->it9300.gator[1].booted = True;
		} 
		else
		{
			DC->it9300.gator[1].booted = False; 
		}
		
		if(DC->it9300.gator[1].booted){
			/* getFirmwareVersion() */
			error = DRV_getFirmwareVersionFromFile(DC, &fileVersion);
			if (error)
			{
			...
			if (cmdVersion != fileVersion)
			{
				...
				error = IT9300_reboot(&DC->it9300, 1);
				DC->bBootCode = true;     
				if (error)
				{
					deb_data("\t Reboot fail [0x%08lx]\n", error);
...
//#endif
		...
		///for TsPort 0
		if(DC->board_id != 48) {
			error = IT9300_setInTsType((Endeavour*) &DC->it9300, br_idx, 4);
			if (error)
			{	
				deb_data("%d it9300 %d Ts Source set TsType error!\n", br_idx, 4);
				return (error);
			}
		} else {
			error = IT9300_setInTsType((Endeavour*) &DC->it9300, br_idx, 0);
			if (error)
			{	
				deb_data("%d it9300 %d Ts Source set TsType error!\n", br_idx, 0);
				return (error);
