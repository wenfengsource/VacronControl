    ...
    IN	Word bandwidth)	// Channel Bandwidth (KHz)
{
	...
	if(handle > 0){
		...
		if (ioctl(handle, IOCTL_ITE_DEMOD_ACQUIRECHANNEL, (void *)&request))
			result = ERR_CANT_FIND_USB_DEV;
		else
			result = request.error;
	}
	...
	OUT	Bool *is_lock)
{
	...
	if(handle > 0){
		if(ioctl(handle, IOCTL_ITE_DEMOD_ISLOCKED, (void *)&request))
			result = ERR_CANT_FIND_USB_DEV;
		else
			result = request.error;
	...
	OUT	PDTVStatistic dtv_statisic)
{
	...
	memset(&channel_statistic, 0, sizeof(channel_statistic));
  memset(&signal_statistic, 0, sizeof(signal_statistic));	
...
Dword DTV_Endeavour_GetBoardID(
		IN int handle,
		OUT Byte *pValue)
{
	Dword result = ERR_NO_ERROR;
	...
	if(handle > 0){
		if(ioctl(handle, IOCTL_ITE_ENDEAVOUR_GETBOARDID, (void *)&byTmpValue))
			result = ERR_CANT_FIND_USB_DEV;
	} else {
		result = ERR_USB_INVALID_HANDLE;
	}
...
Dword DTV_Endeavour_Dibcom_Detail_Channel_Info(
		IN int handle,
		OUT DibcomDiversityData *pValue)
{
	Dword result = ERR_NO_ERROR;

	if(handle > 0){
		if(ioctl(handle, IOCTL_ITE_DIVERSITY_GETDETAILDATA, (void *)pValue))
			result = ERR_CANT_FIND_USB_DEV;
	} else {
		result = ERR_USB_INVALID_HANDLE;
	}
