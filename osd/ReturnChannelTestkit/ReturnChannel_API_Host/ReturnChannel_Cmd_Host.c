	...
	IN	RCHostInfo* deviceInfo)
{
	...
	const Byte* Buffer = deviceInfo->cmdConfig.cmd->buffer_Cmd;
	const Dword bufferLength  = deviceInfo->cmdConfig.cmd->cmdRealSize;
	...
	if( error == ModulatorError_NO_ERROR)
	{
		...
		if(Cmd_checkChecksum(index, index + checkByte - 1, Buffer) != 0)
			error = ReturnChannelError_Reply_WRONG_CHECKSUM;
	}	
	if( error == ModulatorError_NO_ERROR)
	{
		deviceInfo->returnValue.returnCode = Buffer[6];
		command = Buffer[4]<<8 | Buffer[5];

		if(deviceInfo->returnValue.returnCode != 0)
		{
			User_getErrorReply(deviceInfo->device, deviceInfo->returnValue, command);
			error = ReturnChannelError_RerurnCode_Error;
			if((deviceInfo->cmdConfig.IsTimeOut == 0) && (command != CMD_MetadataStreamOutput))
				deviceInfo->cmdConfig.IsParseFinish = 1;
	...
	if( error == ModulatorError_NO_ERROR)
	{
		...
		if((command == CMD_MetadataStreamOutput)|| ((deviceInfo->cmdConfig.IsTimeOut == 0) && ( deviceInfo->cmdConfig.ExpectCmd == command)))
		{
			...
			case CMD_SetEncryptionConfigurationsOutput :
			...
			case CMD_SetMetadataSettingsOutput :
				...
				if(bufferLength >=8)
				{
					...
					else if(valid == UserNameInvalid)
						error = ReturnChannelError_USER_INVALID;
					else if(valid == PasswordInvalid)
						error = ReturnChannelError_PASSWORD_INVALID;
					else
						error = ReturnChannelError_UNDEFINED_INVALID;
				...
				if(bufferLength >=16)
				{
					...
					if(valid == Valid)
					{
						...
						if(deviceInfo->advanceOptions.extensionFlag == 1)
						{
							...
							if(bufferLength >=26)
							{
								check = Cmd_ByteRead(Buffer, checkByte, &index, &deviceInfo->advanceOptions.extensionFlag2, 0);
								if(deviceInfo->advanceOptions.extensionFlag2 == 1)
								{
									check = Cmd_ByteRead(Buffer, checkByte, &index, &deviceInfo->advanceOptions.PCRReStampMode, 0);
								}
							}
						}
						else
			...
			case CMD_GetEncryptionConfigurationOptionsOutput:
				if(bufferLength >=10)
				{
					check = Cmd_StringRead(Buffer , checkByte, &index, &security.userName);
					check = Cmd_StringRead(Buffer , checkByte, &index, &security.password);
					...
					if(valid == Valid)
					{
						check = Cmd_ByteRead(Buffer, checkByte, &index, &deviceInfo->encryptConfigOption.TPSEncryption, 0xFF);
						check = Cmd_ByteRead(Buffer, checkByte, &index, &deviceInfo->encryptConfigOption.DataEncryption, 0xFF);

						User_getEncryptionConfigurationOptionsReply(deviceInfo, command );
					}else if(valid == UserNameInvalid)
						error = ReturnChannelError_USER_INVALID;
					else if(valid == PasswordInvalid)
						error = ReturnChannelError_PASSWORD_INVALID;
			...
			case CMD_GetEncryptionConfigurationsOutput:
				if(bufferLength >=86)
				{
					check = Cmd_StringRead(Buffer , checkByte, &index, &security.userName);
					check = Cmd_StringRead(Buffer , checkByte, &index, &security.password);
					...
					if(valid == Valid)
					{
						...
						check = Cmd_ByteRead(Buffer, checkByte, &index, &deviceInfo->encryptConfig.TPSEncryptionMode, 0xFF);
						check = Cmd_ByteRead(Buffer, checkByte, &index, &deviceInfo->encryptConfig.TPSEncryptionType, 0xFF);
						check = Cmd_BytesRead(Buffer , checkByte,&index, deviceInfo->encryptConfig.TPSEncryptionKey, 8);

						check = Cmd_ByteRead(Buffer, checkByte, &index, &deviceInfo->encryptConfig.DataEncryptionMode, 0xFF);
						check = Cmd_ByteRead(Buffer, checkByte, &index, &deviceInfo->encryptConfig.DataEncryptionType, 0xFF);
						check = Cmd_WordRead(Buffer, checkByte, &index, &deviceInfo->encryptConfig.DataPartialEncryptionSkipLength, 0);
						check = Cmd_BytesRead(Buffer , checkByte,&index, deviceInfo->encryptConfig.DataEncryptionKey, 64);

						User_getEncryptionConfigurationsReply(deviceInfo, command );
					}else if(valid == UserNameInvalid)
						error = ReturnChannelError_USER_INVALID;
					else if(valid == PasswordInvalid)
						error = ReturnChannelError_PASSWORD_INVALID;
			...
			case CMD_SystemRebootOutput:	//SystemRebootReply
				if(bufferLength >=12)
				{
					...
					if(valid == Valid)
					{
						check = Cmd_DwordRead(Buffer , checkByte, &index,  &deviceInfo->systemReboot.rebootDuration, 0xFF);
			...
			case CMD_UpgradeSystemFirmwareOutput:			
				if(bufferLength >=8)
				{
					...
					error = ReturnChannelError_Reply_WRONG_LENGTH;
				break;
			case CMD_MultipleSetCommandApplyOutput:	//MultipleSetCommandApply
				if(bufferLength >=8)
				{
					check = Cmd_StringRead(Buffer , checkByte, &index, &security.userName);
					check = Cmd_StringRead(Buffer , checkByte, &index, &security.password);
					...
					if(valid == Valid)
					{
						check = Cmd_StringClear(&deviceInfo->multipleSetCommandApply.responseMessage);

						check = Cmd_StringRead(Buffer , checkByte, &index, &deviceInfo->multipleSetCommandApply.responseMessage);

						User_multipleSetCommandApplyReply(deviceInfo, command );
					...
					error = ReturnChannelError_Reply_WRONG_LENGTH;
				break;
			case CMD_MultipleSetCommandStartOutput:	//MultipleSetCommandStart
				if(bufferLength >=8)
				{
					check = Cmd_StringRead(Buffer , checkByte, &index, &security.userName);
					check = Cmd_StringRead(Buffer , checkByte, &index, &security.password);
					...
					if(valid == Valid)
					{
						check = Cmd_StringClear(&deviceInfo->multipleSetCommandStart.responseMessage);

						check = Cmd_StringRead(Buffer , checkByte, &index, &deviceInfo->multipleSetCommandStart.responseMessage);

						User_multipleSetCommandStartReply(deviceInfo, command );
			...
			case CMD_GetSerialPortsOutput:	
				if(bufferLength >=9)
				{
					...
					if(valid == Valid)
					{
						...
						{
							check = Cmd_StringClear(&deviceInfo->serialPorts.tokenList[i]);
						}
						...
						check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->serialPorts.serialPortListSize, 0);
						deviceInfo->serialPorts.tokenList = (RCString*) User_memory_allocate( deviceInfo->serialPorts.serialPortListSize * sizeof(RCString));
						for(i=0; i< deviceInfo->serialPorts.serialPortListSize; i++)
						{
							check = Cmd_StringRead(Buffer , checkByte, &index, &deviceInfo->serialPorts.tokenList[i]);
						}

						User_getSerialPortsReply(deviceInfo, command );
					...
					error = ReturnChannelError_Reply_WRONG_LENGTH;
				break;
			case CMD_GetSerialPortConfigurationsOutput:	
				if(bufferLength >=9)
				{
					check = Cmd_StringRead(Buffer , checkByte, &index, &security.userName);
					check = Cmd_StringRead(Buffer , checkByte, &index, &security.password);
					...
					if(valid == Valid)
					{
						...
						{
							check = Cmd_StringClear(&deviceInfo->serialPortConfigList.configList[i].name);
							check = Cmd_StringClear(&deviceInfo->serialPortConfigList.configList[i].token);
							check = Cmd_StringClear(&deviceInfo->serialPortConfigList.configList[i].serialPortToken);
						}
						...
						check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->serialPortConfigList.listSize, 0);
						...
						for(i=0; i< deviceInfo->serialPortConfigList.listSize; i++)
						{
							check = Cmd_StringRead(Buffer , checkByte, &index, &deviceInfo->serialPortConfigList.configList[i].name);
							check = Cmd_StringRead(Buffer , checkByte, &index, &deviceInfo->serialPortConfigList.configList[i].token);
							check = Cmd_StringRead(Buffer , checkByte, &index, &deviceInfo->serialPortConfigList.configList[i].serialPortToken);
							check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->serialPortConfigList.configList[i].type, 0xFF);
							check = Cmd_DwordRead(Buffer , checkByte, &index,  &deviceInfo->serialPortConfigList.configList[i].baudRate, 0xFF);
							check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->serialPortConfigList.configList[i].dataBitLength, 0xFF);
							check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->serialPortConfigList.configList[i].parityBitCheck, 0xFF);
							check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->serialPortConfigList.configList[i].stopBit, 0xFF);
							check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->serialPortConfigList.configList[i].flowControl, 0xFF);

						}

						User_getSerialPortConfigReply(deviceInfo, command );
					...
					error = ReturnChannelError_Reply_WRONG_LENGTH;
				break;
			case CMD_GetSerialPortConfigurationOptionsOutput:	
				if(bufferLength >=15)
				{
					check = Cmd_StringRead(Buffer , checkByte, &index, &security.userName);
					check = Cmd_StringRead(Buffer , checkByte, &index, &security.password);
					...
					if(valid == Valid)
					{
						...
						check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->serialPortConfigOptions.typeOptions, 0xFF);
						check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->serialPortConfigOptions.baudRateAvailableListSize, 0);
						...
						{
							check = Cmd_DwordRead(Buffer , checkByte, &index,  &deviceInfo->serialPortConfigOptions.baudRateList[i], 0xFF);
						}
						check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->serialPortConfigOptions.dataBitLengthMin, 0xFF);
						check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->serialPortConfigOptions.dataBitLengthMax, 0xFF);
						check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->serialPortConfigOptions.parityBitCheckOptions, 0xFF);
						check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->serialPortConfigOptions.stopBitOptions, 0xFF);
						check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->serialPortConfigOptions.flowControlOptions, 0xFF);

						User_getSerialPortConfigOptionsReply(deviceInfo, command );
					...
					error = ReturnChannelError_Reply_WRONG_LENGTH;
				break;
			case CMD_SendReceiveSerialCommandOutput:	//SendReceiveSerialCommandReply
				if(bufferLength >=8)
				{
					check = Cmd_StringRead(Buffer , checkByte, &index, &security.userName);
					check = Cmd_StringRead(Buffer , checkByte, &index, &security.password);
					...
					if(valid == Valid)
					{
						check = Cmd_StringClear(&deviceInfo->serialCommand.receiveSerialData);

						check = Cmd_StringRead(Buffer , checkByte, &index, &deviceInfo->serialCommand.receiveSerialData);

						User_sendReceiveSerialCommandReply(deviceInfo, command );
			...
			case CMD_GetImagingSettingsOutput:	//GetImagingSettingsReply
				if(bufferLength >=108)
				{
					...
					if(valid == Valid)
					{
						...
						User_getImagingSettingsReply(deviceInfo, command );
					}else if(valid == UserNameInvalid)
						error = ReturnChannelError_USER_INVALID;
					else if(valid == PasswordInvalid)
						error = ReturnChannelError_PASSWORD_INVALID;
			...
			case CMD_GetProfilesOutput:	//GetProfilesReply
				if(bufferLength >=16)
				{
					...
					if(valid == Valid)
					{
						...
						check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->mediaProfiles.extension2Flag, 0);
						if(deviceInfo->mediaProfiles.extension2Flag == 1)
						{
							check = Cmd_ByteRead(Buffer , checkByte, &index,  &tmpByte, 0);

							if( tmpByte!= deviceInfo->mediaProfiles.profilesListSize)
							{
								printf("AudioSource Configurations List2 size = %u Not Equl To Profiles List Size!!!\n", tmpByte);
								if(tmpByte < deviceInfo->mediaProfiles.profilesListSize)
								{
									for( i = 0; i < tmpByte; i ++)
									{
										check = Cmd_DwordRead(Buffer, checkByte,&index, &deviceInfo->mediaProfiles.mediaProfilesParam[i].audioSourceInputLevel, 0);
									}
									for( i = tmpByte; i < deviceInfo->mediaProfiles.profilesListSize; i ++)
									{
										deviceInfo->mediaProfiles.mediaProfilesParam[i].audioSourceInputLevel = 0;
									}
								}else
								{
									for( i = 0; i < deviceInfo->mediaProfiles.profilesListSize; i ++)
			...
			case CMD_GetAudioSourceConfigurationsOutput:	//GetAudioSourceConfigurationsReply
				if(bufferLength >=10)
				{
					...
					if(valid == Valid)
					{
						...
						check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->audioSrcConfig.extensionFlag, 0);

						if(deviceInfo->audioSrcConfig.extensionFlag)
						{

							check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->audioSrcConfig.configList2Size, 0);
							for( i = 0; i < deviceInfo->audioSrcConfig.configList2Size; i ++)
							{
								check = Cmd_DwordRead(Buffer , checkByte, &index,  &deviceInfo->audioSrcConfig.audioSrcConfigList[i].inputLevel, 0);
								check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->audioSrcConfig.audioSrcConfigList[i].channels, 1);
							}
						}
			...
			case CMD_GetAudioSourceConfigurationOptionsOutput:	//GetAudioSourceConfigurationOptions Reply
				if(bufferLength >=9)
				{
					...
					if(valid == Valid)
					{
						...
						check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->audioSrcConfigOptions.extensionFlag, 0);
						if(deviceInfo->audioSrcConfigOptions.extensionFlag)
						{
							check = Cmd_DwordRead(Buffer , checkByte, &index,  &deviceInfo->audioSrcConfigOptions.inputLevelRangeMin, 0);
							check = Cmd_DwordRead(Buffer , checkByte, &index,  &deviceInfo->audioSrcConfigOptions.inputLevelRangeMax, 0);
			...
			case CMD_GetRulesOutput:	//GetRules Reply
				if(bufferLength >= 10)
				{
					...
					if(valid == Valid)
					{
						...
						for( i = 0; i <  deviceInfo->ruleList.ruleListSize; i ++)
						{
							...
							}else if(deviceInfo->ruleList.totalRuleList[i].type == 0x15)
							{
								...
								ptrRule_LoiteringDetector = (Rule_LoiteringDetector*) deviceInfo->ruleList.totalRuleList[i].rule;
								check = Cmd_DwordRead(Buffer , checkByte, &index, &ptrRule_LoiteringDetector->timeThreshold, 0xFF);
								check = Cmd_ByteRead(Buffer , checkByte, &index, &ptrRule_LoiteringDetector->isPeriodicNotify, 0);
								check = Cmd_DwordRead(Buffer , checkByte, &index, &ptrRule_LoiteringDetector->timeInterval, 0xFF);
								check = Cmd_ByteRead(Buffer , checkByte, &index, &ptrRule_LoiteringDetector->polylineListSize, 0);
								...
								{
									check = Cmd_WordRead(Buffer , checkByte, &index, &ptrRule_LoiteringDetector->detectPolyLine[j].polygon_x, 0xFF);
									check = Cmd_WordRead(Buffer , checkByte, &index, &ptrRule_LoiteringDetector->detectPolyLine[j].polygon_y, 0xFF);
								}
								check = Cmd_ByteRead(Buffer , checkByte, &index, &ptrRule_LoiteringDetector->metadataStreamSwitch, 0xFF);
					...
					error = ReturnChannelError_Reply_WRONG_LENGTH;
				break;
			case CMD_GetSupportedAnalyticsModulesOutput:	//GetSupportedAnalyticsModules Reply
				if(bufferLength >=9)
				{
					check = Cmd_StringRead(Buffer , checkByte, &index, &security.userName);
					check = Cmd_StringRead(Buffer , checkByte, &index, &security.password);
					...
					if(valid == Valid)
					{
						...
						deviceInfo->supportedModules.analyticsModuleType = NULL;

						check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->supportedModules.supportListSize, 0);
						...
						{
							check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->supportedModules.analyticsModuleType[i], 0xFF);
						}

						User_getSupportedAnalyticsModulesReply(deviceInfo, command );
					...
					error = ReturnChannelError_Reply_WRONG_LENGTH;
				break;
			case CMD_GetAnalyticsModulesOutput:	//GetAnalyticsModules Reply
				if(bufferLength >= 8)
				{
					check = Cmd_StringRead(Buffer , checkByte, &index, &security.userName);
					check = Cmd_StringRead(Buffer , checkByte, &index, &security.password);
					...
					if(valid == Valid)
					{
						for( i = 0; i < deviceInfo->moduleList.moduleListSize; i ++)
						{
							check = Cmd_StringClear(&deviceInfo->moduleList.totalModuleList[i].moduleName);
							check = Cmd_StringClear(&deviceInfo->moduleList.totalModuleList[i].videoSrcToken);
							check = Cmd_StringClear(&deviceInfo->moduleList.totalModuleList[i].moduleToken);
						...
						check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->moduleList.moduleListSize, 0);
						deviceInfo->moduleList.totalModuleList = (TotalModule*) User_memory_allocate( deviceInfo->moduleList.moduleListSize*sizeof(TotalModule));
						for( i = 0; i <  deviceInfo->moduleList.moduleListSize; i ++)
						{
							check = Cmd_StringRead(Buffer , checkByte, &index, &deviceInfo->moduleList.totalModuleList[i].moduleName);
							check = Cmd_StringRead(Buffer , checkByte, &index, &deviceInfo->moduleList.totalModuleList[i].videoSrcToken);
							check = Cmd_StringRead(Buffer , checkByte, &index, &deviceInfo->moduleList.totalModuleList[i].moduleToken);
							check = Cmd_ByteRead(Buffer , checkByte, &index,  &deviceInfo->moduleList.totalModuleList[i].type, 0x00);
							...
							if(deviceInfo->moduleList.totalModuleList[i].type == 0x30)
							{
								...
								ptrAnalyticsModule_ObjectTracker = (AnalyticsModule_ObjectTracker*) deviceInfo->moduleList.totalModuleList[i].module;
								check = Cmd_WordRead(Buffer , checkByte, &index, &ptrAnalyticsModule_ObjectTracker->commandLength, 0);
								check = Cmd_DwordRead(Buffer , checkByte, &index, &ptrAnalyticsModule_ObjectTracker->objectID, 0);
								check = Cmd_ByteRead(Buffer , checkByte, &index, &ptrAnalyticsModule_ObjectTracker->objectTracking, 0);
								check = Cmd_ByteRead(Buffer , checkByte, &index, &ptrAnalyticsModule_ObjectTracker->polygonListSize, 0);
								...
								{
									check = Cmd_WordRead(Buffer , checkByte, &index, &ptrAnalyticsModule_ObjectTracker->detectPolygon[j].polygon_x, 0xFF);
									check = Cmd_WordRead(Buffer , checkByte, &index, &ptrAnalyticsModule_ObjectTracker->detectPolygon[j].polygon_y, 0xFF);
								}
								check = Cmd_ByteRead(Buffer , checkByte, &index, &ptrAnalyticsModule_ObjectTracker->objectClass, 0xFF);
								check = Cmd_ByteRead(Buffer , checkByte, &index, &ptrAnalyticsModule_ObjectTracker->Likelihood, 0xFF);

								ptrAnalyticsModule_ObjectTracker = NULL;
							}
						}

						User_getAnalyticsModulesReply(deviceInfo, command );
				...
				if(bufferLength >=11)
				{
					...
					if(valid == Valid)
					{
						...
						}else if(deviceInfo->metadataStreamInfo.type == 0x10)
						{
							...
							check = Cmd_ByteRead(Buffer , checkByte,&index, &deviceInfo->metadataStreamInfo.metadata_Event.extensionFlag2, 0);
							if(deviceInfo->metadataStreamInfo.metadata_Event.extensionFlag2 == 1)
							{
								check = Cmd_BytesRead(Buffer , checkByte,&index, &deviceInfo->metadataStreamInfo.metadata_Event.TimeStamp, 5);
							}else
							{
								deviceInfo->metadataStreamInfo.metadata_Event.TimeStamp = 0;
							}
						...
						}else if(deviceInfo->metadataStreamInfo.type == 0x15)
						{
							...
							check = Cmd_DwordRead(Buffer , checkByte,&index, &deviceInfo->metadataStreamInfo.metadata_Event.objectID, 0xFF);
							check = Cmd_BytesRead(Buffer , checkByte,&index, &deviceInfo->metadataStreamInfo.metadata_Event.TimeStamp, 5);
							check = Cmd_ByteRead(Buffer , checkByte,&index, &deviceInfo->metadataStreamInfo.metadata_Event.SinceUTCHour, 0xFF);
							check = Cmd_ByteRead(Buffer , checkByte,&index, &deviceInfo->metadataStreamInfo.metadata_Event.SinceUTCMinute, 0xFF);
							check = Cmd_ByteRead(Buffer , checkByte,&index, &deviceInfo->metadataStreamInfo.metadata_Event.SinceUTCSecond, 0xFF);
							check = Cmd_WordRead(Buffer , checkByte,&index, &deviceInfo->metadataStreamInfo.metadata_Event.SinceUTCYear, 0xFF);
							check = Cmd_ByteRead(Buffer , checkByte,&index, &deviceInfo->metadataStreamInfo.metadata_Event.SinceUTCMonth, 0xFF);
							check = Cmd_ByteRead(Buffer , checkByte,&index, &deviceInfo->metadataStreamInfo.metadata_Event.SinceUTCDay, 0xFF);
							check = Cmd_ByteRead(Buffer , checkByte,&index, &deviceInfo->metadataStreamInfo.metadata_Event.UTCHour, 0xFF);
							check = Cmd_ByteRead(Buffer , checkByte,&index, &deviceInfo->metadataStreamInfo.metadata_Event.UTCMinute, 0xFF);
							check = Cmd_ByteRead(Buffer , checkByte,&index, &deviceInfo->metadataStreamInfo.metadata_Event.UTCSecond, 0xFF);
							check = Cmd_WordRead(Buffer , checkByte,&index, &deviceInfo->metadataStreamInfo.metadata_Event.UTCYear, 0xFF);
							...
							User_metadataStreamLoiteringEventReply(deviceInfo, command );
						}else if(deviceInfo->metadataStreamInfo.type == 0x20)
						{
							check = Cmd_StringClear(&deviceInfo->metadataStreamInfo.metadata_SerialData.serialPortToken);
							check = Cmd_StringClear(&deviceInfo->metadataStreamInfo.metadata_SerialData.serialData);

							check = Cmd_StringRead(Buffer , checkByte, &index, &deviceInfo->metadataStreamInfo.metadata_SerialData.serialPortToken);
							check = Cmd_StringRead(Buffer , checkByte, &index, &deviceInfo->metadataStreamInfo.metadata_SerialData.serialData);

							User_metadataSerialDataReply(deviceInfo, command );
						}else if(deviceInfo->metadataStreamInfo.type == 0x21)
						{
							check = Cmd_StringClear(&deviceInfo->metadataStreamInfo.metadata_DigitalInputEvent.token);
							check = Cmd_StringClear(&deviceInfo->metadataStreamInfo.metadata_DigitalInputEvent.data);

							check = Cmd_StringRead(Buffer , checkByte, &index, &deviceInfo->metadataStreamInfo.metadata_DigitalInputEvent.token);
							check = Cmd_ByteRead(Buffer , checkByte,&index, &deviceInfo->metadataStreamInfo.metadata_DigitalInputEvent.logicalState, 0xFF);
							check = Cmd_StringRead(Buffer , checkByte, &index, &deviceInfo->metadataStreamInfo.metadata_DigitalInputEvent.data);

							User_metadataDigitalInputEventReply(deviceInfo, command );
						}else if(deviceInfo->metadataStreamInfo.type == 0x22)
						{
							check = Cmd_StringClear(&deviceInfo->metadataStreamInfo.metadata_DigitalInputEvent.token);
							check = Cmd_StringClear(&deviceInfo->metadataStreamInfo.metadata_DigitalInputEvent.data);

							check = Cmd_StringRead(Buffer , checkByte, &index, &deviceInfo->metadataStreamInfo.metadata_RelayOutputEvent.token);
							check = Cmd_ByteRead(Buffer , checkByte,&index, &deviceInfo->metadataStreamInfo.metadata_RelayOutputEvent.logicalState, 0xFF);
							check = Cmd_StringRead(Buffer , checkByte, &index, &deviceInfo->metadataStreamInfo.metadata_RelayOutputEvent.data);

							User_metadataRelayOutputEventReply(deviceInfo, command );
						}else if(deviceInfo->metadataStreamInfo.type == 0x30)
						{

							check = Cmd_StringClear(&deviceInfo->metadataStreamInfo.metadata_Event.analyticsModuleToken);
							check = Cmd_StringClear(&deviceInfo->metadataStreamInfo.metadata_Event.videoSrcToken);

							check = Cmd_StringRead(Buffer , checkByte, &index, &deviceInfo->metadataStreamInfo.metadata_Event.videoSrcToken);
							check = Cmd_StringRead(Buffer , checkByte, &index, &deviceInfo->metadataStreamInfo.metadata_Event.analyticsModuleToken);
							check = Cmd_BytesRead(Buffer , checkByte,&index, &deviceInfo->metadataStreamInfo.metadata_Event.TimeStamp, 5);
							check = Cmd_ByteRead(Buffer , checkByte,&index, &deviceInfo->metadataStreamInfo.metadata_Event.sceneTooCrowded, 0xFF);
							

							User_metadataStreamObjectTrackerReply(deviceInfo, command );
						}
			...
			case CMD_GetMetadataSettingsOutput:	//GetMetadataSettings Reply
				if(bufferLength >=14)
				{
					...
					if(valid == Valid)
					{
						...
						check = Cmd_ByteRead(Buffer, checkByte, &index, &deviceInfo->metadataSettings.extensionFlag, 0);
						if(deviceInfo->metadataSettings.extensionFlag == 1)
						{
							check = Cmd_ByteRead(Buffer, checkByte, &index, &deviceInfo->metadataSettings.serialDataEnable, 0xFF);
							check = Cmd_WordRead(Buffer, checkByte, &index, &deviceInfo->metadataSettings.serialDataPeriod, 0xFF);
						}
						check = Cmd_ByteRead(Buffer, checkByte, &index, &deviceInfo->metadataSettings.extensionFlag2, 0);
						if(deviceInfo->metadataSettings.extensionFlag2 == 1)
						{
							check = Cmd_ByteRead(Buffer, checkByte, &index, &deviceInfo->metadataSettings.digitalInputEventEnable, 0xFF);
							check = Cmd_WordRead(Buffer, checkByte, &index, &deviceInfo->metadataSettings.digitalInputEventPeriod, 0xFF);
							check = Cmd_ByteRead(Buffer, checkByte, &index, &deviceInfo->metadataSettings.relayOutputEventEnable, 0xFF);
							check = Cmd_WordRead(Buffer, checkByte, &index, &deviceInfo->metadataSettings.relayOutputEventPeriod, 0xFF);
						}
			...
			if((!error) && (command != CMD_MetadataStreamOutput))
				deviceInfo->cmdConfig.IsValid = 1;
			if(command != CMD_MetadataStreamOutput)
			{
				deviceInfo->cmdConfig.IsParseFinish = 1;
				printf("\n------------------------Get command %x---------------------------\n",command);
		...
		{
			if(command != CMD_SetSystemFactoryDefaultBroadcastOutput)
			{
				if(deviceInfo->cmdConfig.IsTimeOut != 0)
				{
					printf("\n Get command %x  BUT IGNORE cause Timeout \n",command);
				}else
				{
...
Dword parseReadyCmd_Host(RCHostInfo* deviceInfo)
{
	Dword error = ModulatorError_NO_ERROR;
	if(deviceInfo->cmdConfig.cmd == NULL)
		return error;

	if(deviceInfo->cmdConfig.cmd->cmdState == RC_CMDINFO_STATE_FINISH)			//vaild command 
	{
		error = Cmd_HostParser(deviceInfo);

		User_memory_free(deviceInfo->cmdConfig.cmd->buffer_Cmd);
		initCmdInfo(deviceInfo->cmdConfig.cmd);
...
Dword Cmd_SetCmdConfigTimeOut (
	...
	if(!error)
	{
		if(command >= CMD_GetMetadataSettingsInput)
			deviceInfo->cmdConfig.ExpectCmd = command | 0xF000;
		else
			deviceInfo->cmdConfig.ExpectCmd = command | 0x8000;
		deviceInfo->cmdConfig.IsTimeOut = 0;

		deviceInfo->cmdConfig.IsParseFinish = 0;
		deviceInfo->cmdConfig.IsValid = 0;

		deviceInfo->callBackSet.ReplyCallBack = callback;
	}
...
Dword Cmd_Send (
	...
	case CMD_GetTPSInformationInput:				
		error = Cmd_generalGet(deviceInfo, command);			
		break;
	case CMD_GetEncryptionConfigurationOptionsInput://GetEncryptionConfigurationOptions				
		error = Cmd_generalGet(deviceInfo, command);			
		break;
	case CMD_GetEncryptionConfigurationsInput:		//GetEncryptionConfigurations		
		error = Cmd_generalGet(deviceInfo, command);			
		break;
	...
	case CMD_SetEITInformationInput:			//SetEITInformation
		...
		break;
	case CMD_SetEncryptionConfigurationsInput:	//SetEncryptionConfigurations
		error = Cmd_setEncryptionConfigurations(deviceInfo, command);
		break;
	...
	case CMD_SetSystemFactoryDefaultInput:	//SetSystemFactoryDefault
		...
		break;
	case CMD_MultipleSetCommandApplyInput:	//MultipleSetCommandApply
		error = Cmd_getWithByte(deviceInfo, deviceInfo->multipleSetCommandApply.mode, command);		
		break;
	case CMD_MultipleSetCommandStartInput:	//MultipleSetCommandStart
		error = Cmd_getWithByte(deviceInfo, deviceInfo->multipleSetCommandStart.mode, command);		
		break;
	...
	case CMD_SetSystemDateAndTimeInput:	//SetSystemDateAndTime
		...
		break;
	case CMD_SetSystemFactoryDefaultBroadcastInput:	//SetSystemFactoryDefaultBroadcast
		error = Cmd_getWithByte(deviceInfo, deviceInfo->systemDefault.factoryDefault, command);
		break;
	...
	case CMD_GetRelayOutputsInput:	//GetRelayOutputs
		error = Cmd_generalGet(deviceInfo, command);		
		break;
	case CMD_GetSerialPortsInput:	//GetSerialPorts
		error = Cmd_generalGet(deviceInfo, command);		
		break;
	case CMD_GetSerialPortConfigurationsInput:	//GetSerialPortConfigurations
		error = Cmd_generalGet(deviceInfo, command);		
		break;
	case CMD_GetSerialPortConfigurationOptionsInput:	//GetSerialPortConfigurationOptions
		error = Cmd_getSerialPortConfigOptions(deviceInfo, command);		
		break;
	...
	case CMD_SetRelayOutputSettingsInput:	//SetRelayOutputSettings
		...
		break;
	case CMD_SendReceiveSerialCommandInput:	//SendReceiveSerialCommand
		error = Cmd_sendReceiveSerialCommand(deviceInfo, command);
		break;
	case CMD_SetSerialPortConfigurationInput:	//SetSerialPortConfig
		error = Cmd_setSerialPortConfig(deviceInfo, command);
		break;
	...
	case CMD_GetRulesInput:	//GetRules
		error = Cmd_generalGet(deviceInfo, command);		
		break;
	case CMD_GetSupportedAnalyticsModulesInput:	//GetSupportedAnalyticsModules
		error = Cmd_generalGet(deviceInfo, command);		
		break;
	case CMD_GetAnalyticsModulesInput:	//GetAnalyticsModules
		error = Cmd_generalGet(deviceInfo, command);		
		break;
	...
	case CMD_DeleteRuleInput:	//DeleteRule
		error = Cmd_deleteRule(deviceInfo, command);
		break;
	case CMD_CreateAnalyticsModuleInput:	//CreateAnalyticsModule
		error = Cmd_createAnalyticsModule(deviceInfo, command);
		break;
	case CMD_ModifyAnalyticsModuleInput:	//ModifyAnalyticsModule
		error = Cmd_modifyAnalyticsModule(deviceInfo, command);
		break;
	case CMD_DeleteAnalyticsModuleInput:	//DeleteAnalyticsModule
		error = Cmd_deleteAnalyticsModule(deviceInfo, command);
		break;
...
Dword Cmd_generalGet (
	...
	Dword	error = ModulatorError_NO_ERROR;
	Byte* cmd_buffer ;
	Dword	payload_start_point,payload_end_point;
	Dword	bufferLength = 0;
	int		txLen;
	Dword cmd_buffer_size = 8;
	Dword cmdsize = 0;
	...
	cmd_buffer = (Byte*)User_memory_allocate((cmd_buffer_size)*sizeof(Byte));
...
Dword Cmd_setAdvanceOptions (
	...
	cmd_buffer[bufferLength++] = advanceOptions->extensionFlag2;
	cmd_buffer[bufferLength++] = advanceOptions->PCRReStampMode;
...
Dword Cmd_setEncryptionConfigurations  (
	...
	int		txLen;
	Byte	i;
	Dword cmd_buffer_size = 86;
   	Dword cmdsize = 0;

	const Security* security = &deviceInfo->security;
	Device* device = &deviceInfo->device;	
	CmdSendConfig* cmdSendConfig = &deviceInfo->cmdSendConfig;
	const EncryptConfig* encryptConfig = &deviceInfo->encryptConfig;
	...
	cmd_buffer[bufferLength++] = encryptConfig->TPSEncryptionMode;
	cmd_buffer[bufferLength++] = encryptConfig->TPSEncryptionType;
	for(i=0;i<8;i++)
		cmd_buffer[bufferLength++] = encryptConfig->TPSEncryptionKey[i];

	cmd_buffer[bufferLength++] = encryptConfig->DataEncryptionMode;
	cmd_buffer[bufferLength++] = encryptConfig->DataEncryptionType;
	Cmd_WordAssign(cmd_buffer, encryptConfig->DataPartialEncryptionSkipLength,&bufferLength);
	for(i=0;i<64;i++)
		cmd_buffer[bufferLength++] = encryptConfig->DataEncryptionKey[i];
...
Dword Cmd_upgradeSystemFirmware(
   ...
   Dword	error = ModulatorError_NO_ERROR;
	...
	Cmd_DwordAssign(cmd_buffer,systemFirmware->totalDataLength,&bufferLength);
	Cmd_DwordAssign(cmd_buffer,systemFirmware->totalDataBlockNumber,&bufferLength);
	Cmd_DwordAssign(cmd_buffer,systemFirmware->dataBlockNumber,&bufferLength);
...
Dword Cmd_setSerialPortConfig(
	...
	Dword	bufferLength = 0;
	int		txLen;
	Dword cmd_buffer_size = 17;
	Dword cmdsize = 0;

	const Security* security = &deviceInfo->security;
	Device* device = &deviceInfo->device;	
	CmdSendConfig* cmdSendConfig = &deviceInfo->cmdSendConfig;
	const SerialPortConfig* serialPortConfig = &deviceInfo->serialPortConfig;

	cmd_buffer_size = cmd_buffer_size + (4 + security->userName.stringLength) + (4 + security->password.stringLength);
	cmd_buffer_size = cmd_buffer_size + (4 + serialPortConfig->name.stringLength) + (4 + serialPortConfig->token.stringLength) + (4 + serialPortConfig->serialPortToken.stringLength);
	cmd_buffer = (Byte*)User_memory_allocate((cmd_buffer_size)*sizeof(Byte));
	cmdsize = cmd_buffer_size - 1;
	...
	Cmd_StringAssign(cmd_buffer, &serialPortConfig->name, &bufferLength);
	Cmd_StringAssign(cmd_buffer, &serialPortConfig->token, &bufferLength);
	Cmd_StringAssign(cmd_buffer, &serialPortConfig->serialPortToken, &bufferLength);
	cmd_buffer[bufferLength++] = serialPortConfig->type;
	Cmd_DwordAssign(cmd_buffer,serialPortConfig->baudRate,&bufferLength);
	cmd_buffer[bufferLength++] = serialPortConfig->dataBitLength;
	cmd_buffer[bufferLength++] = serialPortConfig->parityBitCheck;
	cmd_buffer[bufferLength++] = serialPortConfig->stopBit;
	cmd_buffer[bufferLength++] = serialPortConfig->flowControl;
...
Dword Cmd_sendReceiveSerialCommand(
	...
	Dword	bufferLength = 0;
	int		txLen;
	Dword cmd_buffer_size = 13;
	Dword cmdsize = 0;

	const Security* security = &deviceInfo->security;
	Device* device = &deviceInfo->device;	
	CmdSendConfig* cmdSendConfig = &deviceInfo->cmdSendConfig;
	const SerialCommand* serialCommand = &deviceInfo->serialCommand;

	cmd_buffer_size = cmd_buffer_size + (4 + security->userName.stringLength) + (4 + security->password.stringLength);
	cmd_buffer_size = cmd_buffer_size + (4 + serialCommand->serialPortToken.stringLength)  + (4 + serialCommand->serialData.stringLength)  + (4 + serialCommand->delimiter.stringLength) ;
	cmd_buffer = (Byte*)User_memory_allocate((cmd_buffer_size)*sizeof(Byte));
	cmdsize = cmd_buffer_size - 1;
	...
	Cmd_StringAssign(cmd_buffer, &serialCommand->serialPortToken, &bufferLength);
	Cmd_StringAssign(cmd_buffer, &serialCommand->serialData, &bufferLength);
	cmd_buffer[bufferLength++] = serialCommand->timeOut;
	Cmd_DwordAssign(cmd_buffer,serialCommand->receiveDataLength,&bufferLength);
	Cmd_StringAssign(cmd_buffer, &serialCommand->delimiter, &bufferLength);
...
Dword Cmd_getSerialPortConfigOptions(
   ...
   Dword	error = ModulatorError_NO_ERROR;
	...
	Device* device = &deviceInfo->device;	
	CmdSendConfig* cmdSendConfig = &deviceInfo->cmdSendConfig;
	const SerialPortConfigOptions* serialPortConfigOptions = &deviceInfo->serialPortConfigOptions;

	cmd_buffer_size = cmd_buffer_size + (4 + security->userName.stringLength) + (4 + security->password.stringLength);
	cmd_buffer_size = cmd_buffer_size + (4 + serialPortConfigOptions->serialPortConfigToken.stringLength) ;
	cmd_buffer = (Byte*)User_memory_allocate((cmd_buffer_size)*sizeof(Byte));
	cmdsize = cmd_buffer_size - 1;
	...
	Cmd_StringAssign(cmd_buffer, &serialPortConfigOptions->serialPortConfigToken, &bufferLength);
...
Dword Cmd_setAudioSrcConfig(
   ...
   Dword	error = ModulatorError_NO_ERROR;
	...
	cmd_buffer_size += 1;
	if(audioSrcConfig->extensionFlag)
	{
		cmd_buffer_size = cmd_buffer_size + 4 + 1;
	}
	cmd_buffer = (Byte*)User_memory_allocate((cmd_buffer_size)*sizeof(Byte));
	cmdsize = cmd_buffer_size - 1;
	...
	cmd_buffer[bufferLength++] = audioSrcConfig->extensionFlag;

	if(audioSrcConfig->extensionFlag)
	{
		Cmd_DwordAssign(cmd_buffer,audioSrcConfig->inputLevel,&bufferLength);
		cmd_buffer[bufferLength++] = audioSrcConfig->channels;
	}
...
Dword Cmd_getVideoOSDConfig(
   ...
   error = Cmd_getWithString(deviceInfo,  &videoOSDConfig->videoSrcToken ,command);
...
Dword Cmd_getVideoPrivateArea(
   ...
   error = Cmd_getWithString(deviceInfo,  &videoPrivateArea->videoSrcToken ,command);
...
Dword Cmd_createRule(
   ...
   Dword	error = ModulatorError_NO_ERROR;
	...
	}else if(totalRule->type == 0x14)
	{
		...
		ptrRule_CellMotion = NULL;
	}else if(totalRule->type == 0x15)
	{
		ptrRule_LoiteringDetector = (Rule_LoiteringDetector*) totalRule->rule;
		cmd_buffer_size = cmd_buffer_size + 11 + ( ptrRule_LoiteringDetector->polylineListSize * 4 ) ;
		ptrRule_LoiteringDetector = NULL;
	}else 
	{
	...
	{
		ptrRule_LoiteringDetector = (Rule_LoiteringDetector*) totalRule->rule;
		Cmd_DwordAssign(cmd_buffer,  ptrRule_LoiteringDetector->timeThreshold,&bufferLength);
		cmd_buffer[bufferLength++] = ptrRule_LoiteringDetector->isPeriodicNotify;
		Cmd_DwordAssign(cmd_buffer,  ptrRule_LoiteringDetector->timeInterval,&bufferLength);
		cmd_buffer[bufferLength++] = ptrRule_LoiteringDetector->polylineListSize;
		for( i = 0; i < ptrRule_LoiteringDetector->polylineListSize; i ++)
		{
			Cmd_WordAssign(cmd_buffer,  ptrRule_LoiteringDetector->detectPolyLine[i].polygon_x,&bufferLength);
			Cmd_WordAssign(cmd_buffer,  ptrRule_LoiteringDetector->detectPolyLine[i].polygon_y,&bufferLength);
		}
		cmd_buffer[bufferLength++] = ptrRule_LoiteringDetector->metadataStreamSwitch;
		ptrRule_LoiteringDetector = NULL;
	}
...
Dword Cmd_deleteRule(
   ...
   Dword	error = ModulatorError_NO_ERROR;
	...
	Device* device = &deviceInfo->device;	
	CmdSendConfig* cmdSendConfig = &deviceInfo->cmdSendConfig;
	const DeleteRule* deleteRule = &deviceInfo->deleteRule;

	cmd_buffer_size = cmd_buffer_size + (4 + security->userName.stringLength) + (4 + security->password.stringLength);
	cmd_buffer_size = cmd_buffer_size + (4 + deleteRule->ruleToken.stringLength) + (4 + deleteRule->VideoSourceToken.stringLength) ;
	...
	Cmd_StringAssign(cmd_buffer, &deleteRule->ruleToken, &bufferLength);

	cmd_buffer[bufferLength++] = deleteRule->extensionFlag;
	Cmd_StringAssign(cmd_buffer, &deleteRule->VideoSourceToken, &bufferLength);
...
Dword Cmd_createAnalyticsModule(
   ...
   Dword	error = ModulatorError_NO_ERROR;
	...
	Device* device = &deviceInfo->device;	
	CmdSendConfig* cmdSendConfig = &deviceInfo->cmdSendConfig;
	const TotalModule* totalModule = &deviceInfo->totalModule;

	cmd_buffer_size = cmd_buffer_size + (4 + security->userName.stringLength) + (4 + security->password.stringLength);
	cmd_buffer_size = cmd_buffer_size + (4 + totalModule->moduleName.stringLength) + (4 + totalModule->moduleToken.stringLength) + (4 + totalModule->videoSrcToken.stringLength) ;

	if(totalModule->type == 0x30)
	{
		ptrAnalyticsModule_ObjectTracker = (AnalyticsModule_ObjectTracker*) totalModule->module;
		cmd_buffer_size = cmd_buffer_size + 10 + ( ptrAnalyticsModule_ObjectTracker->polygonListSize * 4 ) ;
		ptrAnalyticsModule_ObjectTracker = NULL;
	}else 
	{
		error = ReturnChannelError_CMD_CONTENT_ERROR;
	...
	Cmd_StringAssign(cmd_buffer, &totalModule->moduleName, &bufferLength);
	Cmd_StringAssign(cmd_buffer, &totalModule->videoSrcToken, &bufferLength);
	Cmd_StringAssign(cmd_buffer, &totalModule->moduleToken, &bufferLength);
	cmd_buffer[bufferLength++] = totalModule->type;

	if(totalModule->type == 0x30)
	{
		ptrAnalyticsModule_ObjectTracker = (AnalyticsModule_ObjectTracker*) totalModule->module;
		Cmd_WordAssign(cmd_buffer, ptrAnalyticsModule_ObjectTracker->commandLength, &bufferLength);
		Cmd_DwordAssign(cmd_buffer, ptrAnalyticsModule_ObjectTracker->objectID, &bufferLength);
		cmd_buffer[bufferLength++] = ptrAnalyticsModule_ObjectTracker->objectTracking;
		cmd_buffer[bufferLength++] = ptrAnalyticsModule_ObjectTracker->polygonListSize;
		for( i = 0; i < ptrAnalyticsModule_ObjectTracker->polygonListSize; i ++)
		{
			Cmd_WordAssign(cmd_buffer,  ptrAnalyticsModule_ObjectTracker->detectPolygon[i].polygon_x,&bufferLength);
			Cmd_WordAssign(cmd_buffer,  ptrAnalyticsModule_ObjectTracker->detectPolygon[i].polygon_y,&bufferLength);
		}
		cmd_buffer[bufferLength++] = ptrAnalyticsModule_ObjectTracker->objectClass;
		cmd_buffer[bufferLength++] = ptrAnalyticsModule_ObjectTracker->Likelihood;
		ptrAnalyticsModule_ObjectTracker = NULL;
	}
...
Dword Cmd_deleteAnalyticsModule(
   ...
   Dword	error = ModulatorError_NO_ERROR;
    //Word    command = 0x068A;
   const DeleteModule* deleteModule = &deviceInfo->deleteModule;

	RCString RCStringArrayTmp[2];
	RCStringArrayTmp[0] = deleteModule->VideoSourceToken;
	RCStringArrayTmp[1] = deleteModule->moduleToken;
	error = Cmd_getWithStrings(deviceInfo, RCStringArrayTmp, 2, command);
...
Dword Cmd_setMetadataSettings(
   ...
   Dword	error = ModulatorError_NO_ERROR;
	...
	cmd_buffer[bufferLength++] = metadataSettings->extensionFlag;
	cmd_buffer[bufferLength++] = metadataSettings->serialDataEnable;
	Cmd_WordAssign(cmd_buffer, metadataSettings->serialDataPeriod,&bufferLength);
	cmd_buffer[bufferLength++] = metadataSettings->extensionFlag2;
	cmd_buffer[bufferLength++] = metadataSettings->digitalInputEventEnable;
	Cmd_WordAssign(cmd_buffer, metadataSettings->digitalInputEventPeriod,&bufferLength);
	cmd_buffer[bufferLength++] = metadataSettings->relayOutputEventEnable;
	Cmd_WordAssign(cmd_buffer, metadataSettings->relayOutputEventPeriod,&bufferLength);
